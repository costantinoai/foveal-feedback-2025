...
from modules.fmri_helpers import (
    ...
    load_new_fmri_classification_data,
)
...
# ----------------------------------------------------------------------------
config = {
    "data_path": "results/20250105-222726_fmri_mvpa/20250105-222742_Bike-Car-Female-Male.pkl",
    "tdann_csv_path": "results/20250105-222243_Clip_svm/20250105-222315_Clip_noise-10/confusion_matrix_Confusion Matrix - Clip | Noise: 10.csv",
    "clip_csv_path": "results/20250105-222203_TDANN_svm/20250105-222237_TDANN_noise-10/confusion_matrix_Confusion Matrix - Tdann | Noise: 10.csv",
    "bh_data_path": "data/behavioural_acc.csv",
    "output_root": "results",
}

output_dir = os.path.join(config["output_root"], f"{create_run_id()}_fmri-plots")
...
try:
    filtered_data = load_new_fmri_classification_data(os.path.join(config["data_path"]))
except:
    filtered_data = load_fmri_classification_data(os.path.join(config["data_path"]))
...
fig3, axes = plt.subplots(1, 5, figsize=(40, 10))
axes = axes.flatten()
...
all_cms = load_and_add_dnn_confmats(all_cms, config["tdann_csv_path"], config["clip_csv_path"])
...
try:
    fov_data = load_new_fmri_classification_data(
...
data_path = os.path.join(config["data_path"], "behavioural_acc.csv")

# Load and preprocess data
behavioral_data, category_means, overall_mean = load_and_prepare_bh_data(
    config["bh_data_path"], list(SUBCAT_ORDER), runs
)
