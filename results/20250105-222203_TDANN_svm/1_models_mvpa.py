...
for model_name, model_label in config["models_to_evaluate"]:
    ...
    # ------------------------------------------------------------------------
    plot_activations_MDS(
        clean_activations, model_output_dir, f"{model_name.capitalize()} MDS Projection"
    )
    ...
    for noise_level in config["noise_levels"]:
        ...
        report_mvpa_results(
            ...
            title=f"Category level decoding accuracy - {model_name.capitalize()} | Noise: {noise_level}",
            chance_level=0.25,
        ...
        report_mvpa_results(
            ...
            title=f"Sub-category level decoding accuracy - {model_name.capitalize()} | Noise: {noise_level}",
            chance_level=0.25,
            accuracy_col="accuracy",
            true_col="True Labels",
            pred_col="Predicted Labels",
        ...
        save_and_plot_confusion_matrix(
            ...
            title=f"Confusion Matrix - {model_name.capitalize()} | Noise: {noise_level}",
            fmt=".2f",
            colorbar=True,
        )
